      ...
      //function Badword
           ...
           //function antilink 
			...
			switch(command) {
				...
				case 'bug':
                    ...
                    client.sendMessage('6281225524272@s.whatsapp.net', options, text, {quoted: mek})
                    ...
                    break
                      case 'runtime':
						...
						ttuks = `${(`━━━━「 *RUNTIME* 」━━━━\n\n◪ ${kyun(runtime)}`)}`
						client.sendMessage(from, `${ttuks}`, MessageType.text, {quoted: tod})
    						break
           //textmaker
             case 'blackpink':
                case 'neon':
		...
		//nsfw menu
                ...
                case 'cum':
				  ...
				  if (!isGroup) return reply(ind.groupo())
				 if (!isNsfw) return reply(ind.nsfwoff())
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/random2/${command}?apikey=azka25`)
                    client.sendMessage(from, buffer, image, { quoted: mek })
                    await limitAdd(sender)
                    break
                     case 'futanari': 
...
━━━⊱  ⸨ *AZKABOT* ⸩  ⊰━━━`
              ...
              if (isLimit(sender)) return reply(ind.limitend(pusname))
                    ...
                    client.sendMessage(from, get_audio, audio, { mimetype: 'audio/mp4', filename: `${get_info.title}.mp3`, quoted: tod })
